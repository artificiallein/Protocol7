...
| 4 ✅ | Contacts, TOFU, key changes and QR verification core | Unknown/changed keys remain unverified; changed keys require matching QR promotion |
| 5 ✅ | FakeTransport | Opaque envelopes route independently of crypto and can be delayed, dropped, duplicated or modified |
...
Milestones 2–5 are implemented as an unaudited local cryptographic and test-transport path. Milestone 6 is next. The current trust repository is intentionally in-memory; durable storage is required before release. Future milestones still require independent review and OS integration testing before any production privacy claim. Group chats, calls, steganography and a custom ratchet are outside the initial MVP.
